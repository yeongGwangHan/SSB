...
public class OrderStateUpdateWithDeliveryAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ordersDAO.updateOrdersState(orderId, OrdersState.DELIVERY);
		
		OrdersDTO updateDTO = ordersDAO.findById(orderId);
		
		
		if(updateDTO.getOrders_state().equals(OrdersState.DELIVERY)) {
			JSMoveFunction.alertLocation(response, "주문이 배송중으로 변경되었습니다.", "./AdOrderDetail.od?orders_id="+orderId);
		}else {
