...
public class OrderStateUpdateWithDetermineAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ordersDAO.updateOrdersState(orderId, OrdersState.DETERMINE);
		
		OrdersDTO updateDTO = ordersDAO.findById(orderId);
		
		
		if(updateDTO.getOrders_state().equals(OrdersState.DETERMINE)) {
			JSMoveFunction.alertLocation(response, "구매결정 상태로 변경되었습니다.", "./AdOrderDetail.od?orders_id="+orderId);
		}else {
